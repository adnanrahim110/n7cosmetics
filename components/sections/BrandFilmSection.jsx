...
export default function BrandFilmSection() {
  const film = homeContent.brandFilm;
  ...
  return (
    <section className="relative isolate min-h-[70svh] overflow-hidden bg-[#080a0b] text-[#f5eee5] md:min-h-[78svh]">
      ...
      <div className="mx-auto flex min-h-[70svh] max-w-360 flex-col justify-between px-4 py-7 sm:px-8 md:min-h-[78svh] md:py-10 lg:px-12">
        ...
        <div className="grid items-end gap-8 pb-2 md:grid-cols-[1.2fr_0.8fr] md:pb-4">
          <motion.div
            initial={{ clipPath: shouldReduceMotion ? "inset(0)" : "inset(100% 0 0 0)", y: shouldReduceMotion ? 0 : 35 }}
            whileInView={{ clipPath: "inset(0% 0 0 0)", y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: shouldReduceMotion ? 0 : 1.15, ease }}
          >
            <h2 className="max-w-4xl font-heading text-[clamp(3.25rem,8vw,8.5rem)] uppercase leading-[0.82] tracking-[0.025em] text-[#f4eadf]">
              {film.title.lead}
              <span className="block font-light italic lowercase tracking-normal text-[#d6ad7c]">{film.title.accent}</span>
            </h2>
          </motion.div>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: shouldReduceMotion ? 0 : 0.9, delay: shouldReduceMotion ? 0 : 0.16, ease }}
