...
export default async function StripeSettings({ saved, error }: { saved?: string; error?: string }) {
  const rows = await selectRows<RowDataPacket & { setting_key: string; value_json: string }>("SELECT setting_key, value_json FROM site_settings WHERE setting_group = 'stripe'");
  const settings = Object.fromEntries(rows.map((row) => [row.setting_key, JSON.parse(row.value_json)]));
  ...
  return <section className="mt-8 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm" id="stripe">
    ...
    <form action={saveStripeSettingsAction} className="mt-5 grid gap-5 sm:grid-cols-2">
      <label className="flex items-center gap-2 text-sm sm:col-span-2"><input type="checkbox" name="stripeEnabled" defaultChecked={settings["stripe.enabled"] === true} />Enable Stripe checkout</label>
      <label className="text-sm font-medium text-zinc-700">Mode<select className={input} name="stripeMode" defaultValue={settings["stripe.mode"] || "test"}><option value="test">Test</option><option value="live">Live</option></select></label>
      <label className="text-sm font-medium text-zinc-700">Publishable key<input className={input} name="stripePublishableKey" defaultValue={settings["stripe.publishable_key"] || ""} placeholder="pk_test_… or pk_live_…" maxLength={500} autoComplete="off" /></label>
