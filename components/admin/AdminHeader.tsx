...
const accountLinks = [
  {
    href: "/admin/profile",
    label: "Profile",
    description: "Your details & password",
    icon: UserRound,
  },
  {
    href: "/admin/settings",
...
function AccountAvatar({ large = false }: { large?: boolean }) {
  return (
    <span
      aria-hidden="true"
      className="grid shrink-0 place-items-center rounded-md p-1.5 bg-primary-500"
    >
      <img
        src="/imgs/fav.png"
        alt="N7 Cosmetics"
        className={cn("h-auto invert-100", large ? "w-7" : "w-4.5")}
      />
    </span>
  );
}
...
function AccountMenu({ administrator, logoutAction }: AdminHeaderProps) {
  ...
  const { portalTarget, placement, style } = useBodyAnchoredDropdown(
    open,
    triggerRef,
    {
      align: "end",
  ...
  const visibleLinks = accountLinks.filter(
    (item) =>
      item.href !== "/admin/settings" || administrator.role !== "FULFILLMENT",
  );
  ...
  return (
    <>
      <button
        ...
        className={`group flex min-w-0 max-w-full cursor-pointer items-center gap-1.5 rounded-xl border py-1.5 pl-1.5 pr-3 text-left outline-none transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-amber-600/40 focus-visible:ring-offset-2 motion-reduce:transition-none ${open ? "border-amber-200/50 bg-amber-50/50" : "bg-zinc-50 border-zinc-100 hover:border-zinc-200 hover:bg-zinc-100"}`}
        id={triggerId}
        onClick={() => {
      ...
      >
        ...
        <ChevronDown
          aria-hidden="true"
          className={`ml-1 shrink-0 text-zinc-400 transition-transform duration-200 group-hover:text-zinc-600 motion-reduce:transition-none ${open ? "rotate-180" : ""}`}
          size={15}
          strokeWidth={1.7}
        />
      </button>
        ...
        ? createPortal(
            <motion.div
              ...
              initial={
                reducedMotion
                  ? false
                  : { opacity: 0, y: placement === "top" ? 5 : -5, scale: 0.98 }
              }
                  ...
                  {visibleLinks.map(
                    ({ href, label, description, icon: Icon }) => (
                      ...
                      >
                        ...
                        <ChevronRight
                          aria-hidden="true"
                          className="text-zinc-300 transition-colors group-hover:text-amber-600"
                          size={14}
                        />
                      </Link>
                    ),
