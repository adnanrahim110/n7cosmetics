...
const inputClass = "mt-1.5 w-full border border-black/15 bg-white px-3 py-2.5 text-sm text-[#1c1814] outline-none transition placeholder:text-black/30 focus:border-[#967c55] focus:ring-2 focus:ring-[#967c55]/12 disabled:cursor-not-allowed disabled:opacity-55";
...
export default function ReviewForm({ productId, productSlug, productName }: { productId: string; productSlug: string; productName: string }) {
  ...
  return (
    <form action={formAction} id="write-review" ref={formRef}>
      <input name="productId" type="hidden" value={productId} />
      <input name="productSlug" type="hidden" value={productSlug} />
      <input name="rating" type="hidden" value={rating || ""} />

      <div aria-live="polite" className={state.status === "idle" ? "sr-only" : `mb-5 flex items-start gap-3 border px-4 py-3 text-sm leading-6 ${state.status === "success" ? "border-emerald-700/25 bg-emerald-50 text-emerald-900" : "border-red-700/20 bg-red-50 text-red-800"}`} role={state.status === "error" ? "alert" : "status"}>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.7fr_1.3fr] lg:gap-8">
        <div className="min-w-0">
          <Title
            ...
            variant="small"
          />
          <p className="mt-2 max-w-md text-sm leading-6 text-black/55">Share your experience. Reviews are checked before publication.</p>

          <fieldset className="mt-5">
            <legend className="text-[11px] font-semibold uppercase tracking-[0.16em] text-black/58">Your rating</legend>
            <div aria-label="Choose a rating" className="mt-1 flex w-fit gap-1" onMouseLeave={() => setHoveredRating(0)} role="radiogroup">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
          ...
          <div className="mt-5">
            ...
            <button
              className={`mt-2 flex min-h-20 w-full items-center gap-3 border border-dashed px-3 py-3 text-left transition ${dragging ? "border-[#8d6745] bg-[#eee4d6]" : "border-black/20 bg-white hover:border-[#8d6745] hover:bg-black/[0.02]"}`}
              disabled={disabled}
              ...
              type="button"
            >
              <UploadCloud className="shrink-0 text-[#8d6745]" size={21} />
            ...
            {files.length ? (
              <div className="mt-3 flex flex-wrap gap-2">
                {files.map((item) => (
                  <div className="group relative size-20 shrink-0 overflow-hidden bg-black/5" key={item.key}>
                    {item.type === "image" ? <Image alt={item.file.name} className="object-cover" fill sizes="80px" src={item.previewUrl} unoptimized /> : <><video className="size-full object-cover" muted preload="metadata" src={item.previewUrl} /><FileVideo2 className="absolute bottom-2 left-2 text-white drop-shadow" size={17} /></>}
        ...
        <div className="grid min-w-0 content-start gap-4">
          ...
          <label className="text-[11px] font-semibold uppercase tracking-[0.16em] text-black/58">Your review<textarea aria-invalid={Boolean(state.fieldErrors?.body)} className={`${inputClass} min-h-28 resize-y leading-6`} disabled={disabled} maxLength={3000} minLength={20} name="body" placeholder="How did the fragrance wear? What stood out to you?" required rows={4} /><FieldError id="review-body-error" message={state.fieldErrors?.body} /></label>
          ...
          <button className="group inline-flex min-h-11 w-fit items-center justify-center gap-2 bg-[#1c1814] px-5 py-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-white transition hover:bg-[#8d6745] disabled:cursor-not-allowed disabled:opacity-45" disabled={disabled} type="submit">
