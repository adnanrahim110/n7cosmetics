...
interface ProductRow extends RowDataPacket { id: string; slug: string; name: string; product_type: string; short_description: string | null; description: string | null; brand: string | null; inspired_by: string | null; audience: string; fragrance_notes_json: unknown; price_pence: number; variant_title: string | null; image_url: string | null; average_rating: number | string }
...
export async function getGlobalStorefrontContent(): Promise<{ header: HeaderContent; footer: FooterContent }> {
  ...
  const availableByHref = new Map(availableSales.map((sale) => [sale.href, sale]));
  header.navigation = header.navigation.flatMap((item) => {
    if (item.href !== "/sale") return [item];
    if (!availableSales.length) return [];
    const selected = item.items?.flatMap((subItem) => {
      const sale = availableByHref.get(subItem.href);
      return sale ? [{ name: sale.name, href: sale.href }] : [];
    }) ?? [];
    return [{
      label: item.label || "Sale",
      href: "/sale",
      type: "dropdown" as const,
      items: selected.length ? selected : availableSales.map((sale) => ({ name: sale.name, href: sale.href })),
    }];
  });
...
export async function getHomepageConfiguration(): Promise<HomepageConfiguration> {
  ...
  config.features.items = Array.isArray(config.features.items) && config.features.items.length
    ? config.features.items
    : defaultHomepageConfiguration.features.items;
