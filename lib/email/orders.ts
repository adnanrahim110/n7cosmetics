...
interface OrderRow extends RowDataPacket {
  ...
  shipping_method_name: string | null; postage_service: string | null; tracking_reference: string | null; tracking_url: string | null;
}
interface ItemRow extends RowDataPacket { product_name: string; variant_title: string; quantity: number; unit_price_pence: number; line_total_pence: number; image_url: string | null }
...
export async function enqueueOrderEmails(orderId: string, event: "confirmation" | string, connection: PoolConnection): Promise<void> {
  ...
  const items = await selectRows<ItemRow>("SELECT product_name, variant_title, quantity, unit_price_pence, line_total_pence, image_url FROM order_items WHERE order_id = ? ORDER BY id", [orderId], connection);
  ...
  const data: OrderEmailData = { number: order.order_number, name: order.customer_name, email: order.customer_email, currency: order.currency, status: order.status, paymentStatus: order.payment_status, fulfillmentStatus: order.fulfillment_status, paymentMethod: order.payment_provider, subtotal: order.subtotal_pence, discount: order.discount_pence, shipping: order.shipping_pence, tax: order.tax_pence, total: order.total_pence, shippingMethod: order.shipping_method_name || undefined, postageService: order.postage_service || undefined, trackingReference: order.tracking_reference || undefined, trackingUrl: order.tracking_url || undefined, bankInstructions: brand.bankInstructions, address: address ? [address.full_name, address.company, address.line_1, address.line_2, address.city, address.region, address.postal_code, address.country_code].filter(Boolean).join("\n") : "No delivery address recorded.", items: items.map((item) => ({ name: item.product_name, variant: item.variant_title, quantity: item.quantity, unitPrice: item.unit_price_pence, total: item.line_total_pence, imageUrl: item.image_url })) };
  const kind = event === "confirmation" ? "confirmation" : "update";
  await enqueueEmail({ ...orderEmail(brand, data, kind), to: order.customer_email, replyTo: brand.replyToEmail, templateKey: kind === "confirmation" ? "order-confirmation" : "order-status-update" }, { dedupeKey: `order:${orderId}:${event}:customer` }, connection);
  await enqueueStoreNotification(brand, kind === "confirmation" ? "new_order" : "order_update", { ...orderEmail(brand, data, kind === "confirmation" ? "team" : "team-update"), replyTo: order.customer_email, templateKey: kind === "confirmation" ? "new-order-team" : "order-update-team" }, `order:${orderId}:${event}`, connection);
}
