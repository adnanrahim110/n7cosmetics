...
import OrderFinancialHistory from "@/components/admin/OrderFinancialHistory";
import OrderShippingDetails from "@/components/admin/OrderShippingDetails";
import StatusBadge from "@/components/admin/StatusBadge";
import { isDatabaseId } from "@/lib/admin/form";
import { selectOne, selectRows } from "@/lib/db/query";
import { updateOrderAction } from "../actions";

interface Order extends RowDataPacket { id: string; source: string; import_id: string | null; legacy_id: string | null; original_status: string | null; customer_id: string | null; order_number: string; status: string; payment_status: string; fulfillment_status: string; currency: string; customer_email: string; customer_name: string; customer_phone: string | null; subtotal_pence: number; discount_pence: number; shipping_pence: number; tax_pence: number; total_pence: number; coupon_code: string | null; customer_notes: string | null; admin_notes: string | null; payment_provider: string | null; payment_reference: string | null; tracking_reference: string | null; tracking_url: string | null; placed_at: Date }
...
const orderStatuses = ["NEW", "CONFIRMED", "PROCESSING", "SHIPPED", "DELIVERED", "COMPLETED", "CANCELLED", "REFUNDED", "FAILED", "ON_HOLD"].map((value) => ({ value, label: value.toLowerCase() }));
...
export default async function OrderDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  return <div>
    ...
    <PageHeader eyebrow="Order" title={order.order_number} description={`Placed ${new Intl.DateTimeFormat("en-GB", { dateStyle: "full", timeStyle: "short" }).format(new Date(order.placed_at))}`} actions={<><LegacyBadge source={order.source} importId={order.import_id} /><StatusBadge status={order.status} /><StatusBadge status={order.payment_status} /></>} />
    {order.source === "LEGACY" ? <Notice type="success">Historical order #{order.legacy_id} · original status: {order.original_status}. Updates will not send customer emails.</Notice> : null}
    ...
    <div className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="space-y-5">
        ...
        <OrderFinancialHistory orderId={id} currency={order.currency} />
        <OrderShippingDetails snapshot={order.shipping_snapshot_json} currency={order.currency} />
      </div>
      <aside className="space-y-5">
        <form action={updateOrderAction.bind(null, id)} className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="font-body text-base font-semibold text-zinc-950">Update order</h2><CustomSelect className="mt-4" defaultValue={order.status} label="Order status" name="status" options={orderStatuses} required searchable={false} />{order.payment_provider === "STRIPE" ? <><input type="hidden" name="paymentStatus" value={order.payment_status} /><p className="mt-4 text-sm text-zinc-600">Payment: {order.payment_status.toLowerCase().replaceAll("_", " ")} · updated by Stripe</p></> : <CustomSelect className="mt-4" defaultValue={order.payment_status} label="Payment" name="paymentStatus" options={paymentStatuses} required searchable={false} />}<CustomSelect className="mt-4" defaultValue={order.fulfillment_status} label="Fulfilment" name="fulfillmentStatus" options={fulfillmentStatuses} required searchable={false} /><label className="mt-4 block text-sm font-medium text-zinc-700">Tracking reference<input className={input} defaultValue={order.tracking_reference ?? ""} maxLength={190} name="trackingReference" /></label><label className="mt-4 block text-sm font-medium text-zinc-700">Tracking URL<input className={input} defaultValue={order.tracking_url ?? ""} maxLength={1000} name="trackingUrl" type="url" /></label><label className="mt-4 block text-sm font-medium text-zinc-700">Internal notes<textarea className={input} defaultValue={order.admin_notes ?? ""} maxLength={10000} name="adminNotes" rows={4} /></label><label className="mt-4 block text-sm font-medium text-zinc-700">Status update note<input className={input} maxLength={500} name="historyNote" /></label><button className="mt-5 w-full rounded-lg bg-zinc-950 px-4 py-2.5 text-sm font-semibold text-white" type="submit">Save order</button></form>
        <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="font-body text-base font-semibold text-zinc-950">Customer</h2><p className="mt-3 text-sm font-medium">{order.customer_name}</p>{order.customer_id ? <Link className="mt-2 block text-sm text-amber-700 underline" href={`/admin/customers/${order.customer_id}`}>Customer profile</Link> : null}<a className="mt-1 block text-sm text-amber-700" href={`mailto:${order.customer_email}`}>{order.customer_email}</a>{order.customer_phone ? <a className="mt-1 block text-sm text-zinc-600" href={`tel:${order.customer_phone}`}>{order.customer_phone}</a> : null}{order.customer_notes ? <p className="mt-4 rounded-lg bg-zinc-50 p-3 text-sm text-zinc-600">{order.customer_notes}</p> : null}</section>
