...
interface Order extends RowDataPacket { id: string; source: string; import_id: string | null; legacy_id: string | null; original_status: string | null; customer_id: string | null; order_number: string; status: OrderStatus; payment_status: string; fulfillment_status: string; currency: string; customer_email: string; customer_name: string; customer_phone: string | null; subtotal_pence: number; discount_pence: number; shipping_pence: number; tax_pence: number; total_pence: number; coupon_code: string | null; customer_notes: string | null; admin_notes: string | null; payment_provider: string | null; payment_reference: string | null; tracking_reference: string | null; postage_service: string | null; placed_at: Date }
...
export default async function OrderDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  return <div>
    ...
    <PageHeader eyebrow="Order" title={order.order_number} description={`Placed ${new Intl.DateTimeFormat("en-GB", { dateStyle: "full", timeStyle: "short" }).format(new Date(order.placed_at))}`} actions={<><LegacyBadge source={order.source} importId={order.import_id} /><OrderStatusControl canCancelOrRefund={administrator.role !== "FULFILLMENT"} historical={order.source === "LEGACY"} orderId={id} orderNumber={order.order_number} postageService={order.postage_service} status={order.status} trackingReference={order.tracking_reference} /><StatusBadge status={order.payment_status} /></>} />
    ...
    <div className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="space-y-5">
        <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="mb-4 font-body text-base font-semibold text-zinc-950">Royal Mail tracking</h2><OrderTrackingForm key={`${order.postage_service}:${order.tracking_reference}`} historical={order.source === "LEGACY"} orderId={id} postageService={order.postage_service} trackingReference={order.tracking_reference} /></section>
        <form key={`${order.status}:${order.payment_status}:${order.fulfillment_status}`} action={updateOrderAction.bind(null, id)} className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="font-body text-base font-semibold text-zinc-950">Update order</h2>{order.payment_provider === "STRIPE" || administrator.role === "FULFILLMENT" ? <><input type="hidden" name="paymentStatus" value={order.payment_status} /><p className="mt-4 text-sm text-zinc-600">Payment: {order.payment_status.toLowerCase().replaceAll("_", " ")} · {order.payment_provider === "STRIPE" ? "updated by Stripe" : "managed by an owner or manager"}</p></> : <CustomSelect className="mt-4" defaultValue={order.payment_status} label="Payment" name="paymentStatus" options={paymentStatuses} required searchable={false} />}<CustomSelect className="mt-4" defaultValue={order.fulfillment_status} label="Fulfilment" name="fulfillmentStatus" options={fulfillmentStatuses} required searchable={false} /><label className="mt-4 block text-sm font-medium text-zinc-700">Internal notes<textarea className={input} defaultValue={order.admin_notes ?? ""} maxLength={10000} name="adminNotes" rows={4} /></label><label className="mt-4 block text-sm font-medium text-zinc-700">Status update note<input className={input} maxLength={500} name="historyNote" /></label><button className="mt-5 w-full rounded-lg bg-zinc-950 px-4 py-2.5 text-sm font-semibold text-white" type="submit">Save order</button></form>
