...
interface Subscriber extends RowDataPacket { id: string; email: string; status: string; requested_at: Date; confirmed_at: Date | null; marketing_basis: "CONSENT" | "SOFT_OPT_IN" | null }
...
export default async function EmailsPage({ searchParams }: { searchParams: Promise<{ tab?: string; page?: string; q?: string; status?: string; saved?: string; error?: string }> }) {
  ...
  const subscribers = tab === "subscribers" ? await selectRows<Subscriber>(`SELECT CAST(id AS CHAR) AS id, email, status, requested_at, confirmed_at, marketing_basis FROM newsletter_subscribers WHERE email LIKE ? ORDER BY requested_at DESC LIMIT 31 OFFSET ${offset}`, [pattern]) : [];
  ...
  return <div>
    ...
    <div className="space-y-3">
      ...
      {subscribers.slice(0, 30).map((subscriber) => <article className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-white p-5" key={subscriber.id}><div><p className="break-all font-medium">{subscriber.email}</p><p className="mt-1 text-xs text-zinc-500">{subscriber.status} · Recorded {date(subscriber.requested_at)}{subscriber.confirmed_at ? ` · Confirmed ${date(subscriber.confirmed_at)}` : ""}</p><p className="mt-1 text-xs text-zinc-500">{subscriber.marketing_basis === "SOFT_OPT_IN" ? "Checkout · Soft opt-in" : subscriber.marketing_basis === "CONSENT" ? "Newsletter · Explicit opt-in" : "Checkout · Opted out"}</p></div>{subscriber.status !== "UNSUBSCRIBED" ? <form action={unsubscribeSubscriberAction.bind(null, subscriber.id)}><button className={button}>Unsubscribe</button></form> : null}</article>)}
